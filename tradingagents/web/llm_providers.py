...
def run_workflow(context: str, mode: str = "both") -> dict:
    ...
    if mode in ("analyst", "both"):
        ...
        analyst_prompt = f"""你是專業金融分析師。我提供你這檔標的的歷史財報與基本面、17D 全景技術矩陣（含歷史偏向）、即時技術指標與大盤環境。請**交叉判讀**後給出操作建議與投資建議。用**繁體中文**。

{context}

**方法**：把財報（營收/EPS/毛利趨勢）、估值（本益比/成長/賣方目標價）、17D 技術（共振區、各維度偏向、歷史偏向變化）三者交叉比對，重點在三者是同向強化還是背離；背離時明確指出並權衡。點位要有依據（進場=技術共振區且估值合理；停損=結構失效；停利=目標價/壓力共振區）。資料缺口降權。

直接給結論（markdown，條列用編號）：

1. **交叉判讀** — 財報 × 估值 × 17D 技術三者的關係與綜合研判（核心）
