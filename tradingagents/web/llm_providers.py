...
def run_workflow(context: str, mode: str = "both") -> dict:
    ...
    if mode in ("analyst", "both"):
        ...
        analyst_prompt = f"""你是專業金融分析師。以下提供即時技術指標、17D 全景技術矩陣與基本面估值，請做**繁體中文**深度分析：

{context}

**方法**：不要只看現價微調點位。請從「技術結構 × 基本面 × 估值」的相對關係推導。技術面以 17D 共振區與系統建議進出場區為依據，估值面對照本益比/成長/賣方目標價判斷貴或便宜；技術與基本面背離時須明確點出並權衡。點位要有依據（進場=技術共振區且估值未過熱；停損=結構失效；停利=目標價/壓力共振區）。
